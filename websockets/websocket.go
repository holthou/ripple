...
func wsHandshakeValidator(allowedOrigins []string) func(*http.Request) bool {
	...
	glog.Infoln(fmt.Sprintf("Allowed origin(s) for WS RPC interface %v", origins.ToSlice()))
	...
	f := func(req *http.Request) bool {
		...
		glog.Warningln("Rejected WebSocket connection", "origin", origin)
		return false
	}
...
func ruleAllowsOrigin(allowedOrigin string, browserOrigin string) bool {
	...
	if err != nil {
		glog.Warningln("Error parsing allowed origin specification", "spec", allowedOrigin, "error", err)
		return false
	}
	browserScheme, browserHostname, browserPort, err = parseOriginURL(browserOrigin)
	if err != nil {
		glog.Warningln("Error parsing browser 'Origin' field", "Origin", browserOrigin, "error", err)
		return false
	}
