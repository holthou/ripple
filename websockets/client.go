...
func (c *Client) Notify(ctx context.Context, args ...interface{}) error {
	op := new(requestOp)
	msg, err := c.newMessage(args)
	if err != nil {
		return err
	...
	return c.send(ctx, op, msg.Command)
}
...
func (c *Client) dispatch(codec ServerCodec) {
	...
	for {
		...
		case err := <-c.readErr:
			glog.Infoln("RPC connection read error", "err", err)
			conn.close(err, lastOp)
			reading = false

		// Reconnect:
		case newcodec := <-c.reconnected:
			glog.Infoln("RPC client reconnected", "reading", reading, "conn", newcodec.remoteAddr())
			if reading {
